...
def read_pdf(files, description, min_summary_length=50, max_summary_length=1000, similarity_threshold=80):
    outputs = []
    ...
    for file in files:
        try:
            ...
            # Generate the summary using customized parameters
            summary = summarizer(preprocessed_text, min_length=min_summary_length, max_length=max_summary_length)[0]["summary_text"]
...
iface = gr.Interface(
    ...
    [
        ...
        gr.inputs.Textbox(label="Description", placeholder="Enter job description or keywords with , separation for checking similarity"),
        gr.inputs.Slider(label="Minimum Summary Length", minimum=10, maximum=500, default=50, step=10),
        gr.inputs.Slider(label="Maximum Summary Length", minimum=100, maximum=1000, default=500, step=50),
        gr.inputs.Slider(label="Similarity Threshold", minimum=0, maximum=100, default=80, step=5),
    ],
